...
def addition(request,x,y):
    z=int(x)+int(y)
    print('Addition is:',z)
    return HttpResponse('Additon is:'+str(z))
'''
...
def product_list(request):
   ...
   # context['plist']=['Iphone','saumsung','Nokia','vivo']
    context['plist']=[
...
def addtocart(request,pid):
    ...
    if request.user.is_authenticated:
        ...
        #check wheather user alredy added product in the cart
        q1=Q(pid=pid)
        q2=Q(uid=userid)
        c=Cart.objects.filter(q1 & q2)#0 or more than 1
...
def viewcart(request):
    ...
    return HttpResponse("cart record fetched")
...
def makepayment(request):
    ...
    client = razorpay.Client(auth=("rzp_test_4Ei3WSC2Rx1cYh", "FdHx1FHcvOsvtp36RTtreidI"))
